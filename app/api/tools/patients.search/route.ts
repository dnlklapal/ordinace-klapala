// app/api/tools/patients.search/route.ts
import { NextResponse } from "next/server";

export const runtime = "nodejs";

const API_KEY = process.env.TOOLS_API_KEY;

const MOCK = [
  { id: "p1", name: "Jan Novák",      phone: "+420777000111", lastVisit: "2025-05-16T09:00:00.000Z" },
  { id: "p2", name: "Eva Dvořáková",  phone: "+420777000222", lastVisit: "2025-06-10T13:30:00.000Z" },
];

export async function POST(req: Request) {
  // API key check
  if (API_KEY && req.headers.get("x-api-key") !== API_KEY) {
    console.log("[TOOLS] patients.search UNAUTHORIZED");
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }

  const body = await req.json().catch(() => ({}));
  const q = (body?.q ?? "").toString().toLowerCase();
  console.log("[TOOLS] patients.search HIT", { q });

  const patients = !q
    ? []
    : MOCK.filter(
        (p) => p.name.toLowerCase().includes(q) || p.phone.replace(/\s+/g, "").includes(q)
      );

  console.log("[TOOLS] patients.search RESPOND", patients.length);
  return NextResponse.json({ patients });
}
